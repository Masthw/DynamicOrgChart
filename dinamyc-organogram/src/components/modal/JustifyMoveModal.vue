<template>
  <q-dialog v-model="isOpen" persistent>
    <q-card class="move-modal">
      <div class="icon-wrapper">
        <div class="icon-container">
          <img src="src/assets/icons/arrow_forward.png" alt="Mover" class="modal-icon" />
        </div>
      </div>
      <q-card-section class="modal-header">
        <h2 class="modal-title">Mover Empregado</h2>
        <p class="modal-text">
          Deseja realmente mover este empregado para os não designados?<br />
      ...
      <q-card-actions class="button-container">
        ...
        <ButtonComponent label="Confirmar" variant="secondary" :disabled="!isFormValid" @click="confirmMove" />
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
